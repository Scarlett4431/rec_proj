...
class RecallDataset(Dataset):
    ...
    Ensures hard negatives are filtered against the user's true positives.
    """

    def __init__(self, ratings_df, user_feat_df, item_feat_df,
                 item_emb=None, hard_neg_k=0, hard_neg_samples=0):
        """
        ratings_df: DataFrame with [user_idx, item_idx]
        user_feat_df, item_feat_df: engineered features
        item_emb: torch.Tensor [num_items+1, d], pretrained embeddings (for mining)
        ...
        # Feature stores
        self.user_store = FeatureStore(user_feat_df, "user_idx")
        self.item_store = FeatureStore(item_feat_df, "item_idx")

        self.user_feat_dim = self.user_store.dim
        self.item_feat_dim = self.item_store.dim

        # Track which items each user consumed (immutable sets)
        user_rated = {}
        ...
        # Precompute item-item neighbors once (global, immutable arrays)
        if hard_neg_k > 0 and item_emb is not None:
            try:
                ...
                for pos_item in unique_pos_items:
                    ...
                    if neighbors:
                        self.hard_negatives[pos_item] = np.array(
                            neighbors, dtype=np.int64
                        )
            except ImportError:
    ...
    def __getitem__(self, idx):
        ...
        i_feats = self.item_store.get(pos_item)

        sample = {
            "user_id": torch.tensor(u, dtype=torch.long),
            "pos_item": torch.tensor(pos_item, dtype=torch.long),
            "user_feats": u_feats.clone(),
            "pos_item_feats": i_feats.clone(),
        }
        ...
        if self.hard_neg_samples > 0 and pos_item in self.hard_negatives:
            ...
            # Filter out items this user actually consumed
            consumed = self.user_rated.get(u, frozenset())
            filtered = np.array([c for c in candidates if c not in consumed],
                                dtype=np.int64)

            if filtered.size == 0:
                neg_items = torch.full((self.hard_neg_samples,), -1, dtype=torch.long)
            else:
                replace = filtered.size < self.hard_neg_samples
                sampled = np.random.choice(
                    filtered, size=self.hard_neg_samples, replace=replace
                )
                neg_items = torch.from_numpy(sampled.astype(np.int64))
            sample["hard_neg_items"] = neg_items
        else:
            sample["hard_neg_items"] = torch.full(
                (self.hard_neg_samples,), -1, dtype=torch.long
            )
